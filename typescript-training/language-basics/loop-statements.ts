...
//     i++;
// } //Infinite time will run 

while (i>0){
    if(isPageloaded || i==5) {
        break;//break the loop intentionally when the page is loaded or when the page is refreshed already 5 times.
    }
    console.log ("Refresh the Page");
    i++;
} 
...
let empNames : string [] = ["jyothi", "swomya", "sravani", "sushma"];
...
//for...of loop
for (let val of empNames) {
    console.log(val);//get each and every value of empNames
}
...
for (let val in person) {
    console.log (val); // get each & every property/key inside the person object.
}
